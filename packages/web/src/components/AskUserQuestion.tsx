...
export function AskUserQuestion({
  ...
  // Reset active tab when pendingQuestion changes (render-phase reset)
          ...
          {questions.map((q, i) => (
            ...
            >
              ...
              {isQuestionAnswered(q) && (
                <span className="ask-question-tab-check">&#10003;</span>
              )}
            </button>
          ))}
        </div>
        ...
        <div className="ask-question-item">
          ...
          <div className="ask-question-text">
            ...
            {currentQuestion.multiSelect && (
              <span className="ask-question-multi-hint"> (multi-select)</span>
            )}
          </div>
          <div className="ask-question-options">
            {currentQuestion.options.map((opt) => {
              ...
              const isSelected =
                sel instanceof Set ? sel.has(opt.label) : sel === opt.label;
              return (
                <button
                  key={opt.label}
      ...
      <div className="ask-question-footer">
        {questions.length > 1 && (
          <span className="ask-question-progress">
            {questions.filter(isQuestionAnswered).length} / {questions.length}{" "}
            answered
          </span>
        )}
        <button
